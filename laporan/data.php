...
<div class='container' onclick="window.print()">
    ...
    <?php
    $x=1;
    $query = mysqli_query($koneksi, "SELECT b.nama_barang, sum(p.jumlah) jumlah FROM tb_penjualan p join tb_barang b WHERE (p.tgl_penjualan BETWEEN '$mulai' AND '$akhir') AND p.kode_barcode = b.kode_barcode GROUP by p.kode_barcode order by jumlah desc limit 10");
    while($row = mysqli_fetch_array($query)){
        if ($x==1) {
            $satu=$row['jumlah'];
    ...
    <script>
        ...
        var myChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: ["Amonia", "Ace", "Yellow", "Green", "Purple", "Orange", "Yellow", "Green", "Purple", "Orange"],
                datasets: [{
                    label: '# of Votes',
                    data: [5, 15, 25, 12, 76, 23, 55, 45, 33, 9],
                    backgroundColor: [
                    'rgba(255, 99, 132, 0.2)',
                    'rgba(48, 9, 99, 0.53)',
                    'rgba(4, 82, 95, 0.66)',
